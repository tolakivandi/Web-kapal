<?php
    ...
    $id = $_GET["id"];
    $query = "SELECT * FROM kapal WHERE id_kapal='$id'";
    ...
    $results_4 = mysqli_query($connection, "SELECT * FROM kapal WHERE id_kapal='$id'");
    
    $items_1 = [];
    $items_2 = [];
    $items_3 = [];
    $items_4 = mysqli_fetch_assoc($results_4); 

    while($result = mysqli_fetch_assoc($results_1)) {
        $items_1[] = $result;
    }
    while($result = mysqli_fetch_assoc($results_2)) {
        $items_2[] = $result;
    }
    while($result = mysqli_fetch_assoc($results_3)) {
        $items_3[] = $result;
    }
?>
<!doctype html>
...
<body>
  <nav class="bg-orange-300 text-white py-4">
    <div class="container mx-auto flex flex-wrap items-center justify-between">
      <a href="" class="flex h-max items-center gap-3 font-semibold">
        ...
        <button items_4-collapse-toggle="mobile-menu" type="button" class="md:hidden ml-3 text-gray-400 hover:text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-300 rounded-lg inline-flex items-center justify-center" aria-controls="mobile-menu-2" aria-expanded="false">
  ...
  <!-- component -->
<form method="post" action="../function/kapal/update_kapal.php" class="w-full overflow-scroll h-screen bg-gradient-to-r from-blue-100 via-purple-100 to-pink-100 p-4 flex items-center justify-center" >
    <div class="bg-white py-6 px-10 sm:max-w-md w-full rounded">
        ...
        <div class="flex flex-col gap-7">
            <div>
                 <input type="text" class="focus:outline-none border-b w-full pb-2 border-sky-400 placeholder-gray-500" name="nama_kapal" value="<?= $items_4["nama_kapal"] ?>"  placeholder="Name "/>
            </div>
            <div class="relative">
                ...
                <option value="<?= $x["id_dpi"] ?>" <?= ($x["id_dpi"] === $items_4["id_dpi"]) ? "selected" : "" ?> ><?= $x["nama_dpi"]?></option>
                <?php endforeach ?>
                </select>
            ...
            <div class="relative">
                ...
                <option value="<?= $x["id_pemilik"] ?>" <?= ($x["id_pemilik"] === $items_4["id_pemilik"]) ? "selected" : "" ?> ><?= $x["nama_pemilik"]?></option>
                <?php endforeach ?>
                </select>
                <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
            ...
            <div class="relative">
                ...
                <option value="<?= $x["id_alat_tangkap"] ?>" <?= ($x["id_alat_tangkap"] === $items_4["id_alat_tangkap"]) ? "selected" : "" ?> ><?= $x["nama_alat_tangkap"]?></option>
                <?php endforeach ?>
                </select>
                <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
            ...
            <input type="hidden" name="id_kapal" value="<?= $items_4["id_kapal"] ?>">
